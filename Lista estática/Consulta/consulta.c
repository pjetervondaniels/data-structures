...
int consultaConteudoLista(Lista *li,int conteudo,struct aluno *al){
    ...
    if(li != NULL){
        int k,i=0;
        while (i<li->qtd && li->dados[i].matricula != conteudo){
            i++;
            if(i == li->qtd){
                return 0 ;
            }
...
void main (){
    ...
    int posicao;
    int d1 = consultaPosicaoLista(li,posicao,&dados_aluno);
     // Consulta feita por posição em uma lista...
    if(d1==1){
        printf("Consulta no meio da lista feita com sucesso..");
    }else{
        printf("Não foi feita Consulta no meio da lista...");
    }

    int conteudo;
    int d2 = consultaConteudoLista(li,conteudo,&dados_aluno);
     // Consulta feita por conteúdo em uma lista...
    if(d1==1){
        printf("Consulta no meio da lista feita com sucesso..");
    }else{
        printf("Não foi feita Consulta no meio da lista...");
